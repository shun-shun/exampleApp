...
public class TodoUpdateService implements FindService {
	...
	public Response execute(Request request) {
		...
		StatusCode statusCode = repository.get("UpdateRepositoryImpl").update(todoData);
		Response response = modeling(statusCode);
		return response;
	}
	
	private TodoData modeling(Request request) {
		if (!(request instanceof TodoUpdatePostRequest)) {
			throw new IllegalArgumentException();
		}

		TodoUpdatePostRequest selectRequest = (TodoUpdatePostRequest) request;
